...
- 출발 30분 이내에는 설정에서 켜고 끌 수 있는 출발 전 상태 알림을 표시하며, T-30/T-15/T-5 재조회 결과로 권장 출발 시각이 바뀌면 같은 알림을 갱신합니다.
...
현재 한계: 앱은 ODsay 후보 경로 안에서 첫 버스 탑승 가능성을 평가하지만, ODsay 결과 밖의 주변 버스 대안을 직접 탐색하지는 않습니다. 서울 버스 도착정보 키와 endpoint는 정상 동작하지만, 일부 경로에서는 ODsay `busID/routeID`가 서울버스 `busRouteId`와 일치하지 않아 정류장 ARS 기준 도착목록 조회로 개선할 필요가 있습니다. 알림 화면의 대안 경로 설명 UI도 후속 작업입니다.
...
- 서울 버스 도착정보의 정류장 ARS 기준 조회 및 노선번호 매칭 fallback
