...
| 버스 실시간 도착 | ODsay 실시간 버스 도착정보 또는 서울특별시 버스도착정보조회 서비스 | 높음 | 첫 탑승 정류장의 실제 대기 시간 보정 |
...
3. 첫 탑승 구간이 버스이면 실시간 버스 도착정보를 조회해 대기 시간을 보정합니다.
4. 첫 탑승 구간이 지하철이면 지하철 실시간 도착정보를 조회해 대기 시간을 보정합니다.
5. 앱 실행 시점 또는 출발 알림 직전에 WorkManager로 경로와 실시간 도착정보를 재조회합니다.
6. 이동 기록 저장 시 목표 대비 도착 오차를 개인 보정값에 반영합니다. 현재는 한 번의 기록이 보정값을 과도하게 흔들지 않도록 최대 ±5분 범위에서 조정합니다.
7. 자동차 모드는 Google Routes `TRAFFIC_AWARE` 옵션을 별도 provider 정책으로 적용합니다.
...
- 실시간 버스 도착정보, 버스 위치정보, 지하철 실시간 도착정보는 아직 provider로 구현되어 있지 않습니다.
...
2. ODsay 경로 응답에서 첫 탑승 정류장/역 정보를 추출해 실시간 도착정보 조회 입력값으로 사용합니다.
3. 버스 실시간 도착정보 provider를 추가해 첫 버스 대기 시간을 보정합니다.
4. 지하철 실시간 도착정보 provider를 추가해 첫 지하철 대기 시간을 보정합니다.
5. Google Routes `arrivalTime` 또는 `departureTime`을 추천 계산 흐름에 맞게 연결하고, 자동차 모드에는 `TRAFFIC_AWARE` 정책을 검토합니다.
6. 실제 API 실패 시 마지막 성공값 캐시를 fallback 후보에 추가합니다.
7. API 실패/좌표 없음 상태를 사용자가 이해할 수 있는 UI 메시지로 분리합니다.
